...
### Stop a workflow session


**Curl**
```bash
curl -X POST \
  'http://<baseUrl>/apis/request/workflow/stop/<id>' \
  --header 'api-key: <api-key>' \
```
...
url = "http://<baseUrl>/apis/request/workflow/stop/<id>"
headers = {
    "api-key": "<api-key>"
}
...
**Javascript**
```javascript
const url = 'http://<baseUrl>/apis/request/workflow/stop/<id>';
const headers = {
    'api-key': '<api-key>'
};
...
public class WorkflowStop {
    public static void main(String[] args) {
        try {
            URL url = new URL("http://<baseUrl>/apis/request/workflow/stop/<id>");
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("POST");
            conn.setRequestProperty("api-key", "<api-key>");
            conn.setDoOutput(true);
